...
(function($) {
  $.tinysort.defaults.attr = "id";
	$.fn.asmSelectEx = function(customOptions) {
		...
		var settings = $.extend({}, options, customOptions); 
		...
		return this.each(function(index) {
			...
			function init() {
					...
					$select = $("<select></select>").addClass(options.selectClass).attr('id', options.selectClass + index); 

					$selectRemoved = $("<select></select>"); 

					$ol = $("<" + options.listType + "></" + options.listType + ">").addClass(options.listClass).attr('id', options.listClass + index); 

					$container = $("<div></div>").addClass(options.containerClass).attr('id', options.containerClass + index); 

					buildSelect();

					$select.change(selectChangeEvent).click(selectClickEvent); 

				if (options.originalChangeEvent == 'reload') {
					$original.change(originalReload).wrap($container).before($select).before($ol);
				} else {
					$original.change(originalAddItem).wrap($container).before($select).before($ol);
				}
			...
			function buildSelect() {
				...
				if(!options.debugMode)
					$original.hide(); // IE6 requires this on every buildSelect()
					selectFirstItem();
			...
			function buildOptions(el, group) {
				el.children("option").each(function(n) {
					...
					if(!$t.attr('id'))
						$t.attr('id', 'asm' + index + group_text + 'option' + n); 
					id = $t.attr('id'); 
			...
			function addSelectOption(optionId, disabled) {
				...
				var $O = $('#' + optionId); 
				var $option = $("<option>" + $O.text() + "</option>").val($O.val()).attr('rel', optionId);
			...
			function addListItem(optionId, position) {
				...
				var $removeLink = $("<a></a>").attr("href", "#").addClass(options.removeClass).prepend(options.removeLabel).click(function() { 
					dropListItem($(this).parent('li').attr('rel')); 
					return false; 
				}); 

				var $itemLabel = $("<span></span>").addClass(options.listItemLabelClass).html($O.html()); 

				var $item = $("<li></li>").attr('rel', optionId).attr('id', $ol.attr('id') + '_' + position).addClass(options.listItemClass).append($itemLabel).append($removeLink).hide();
			...
			function setHighlight($item, label) {
				...
				var $highlight = $("<span></span>").hide().addClass(options.highlightClass).attr('id', options.highlightClass + index).html(label + $item.children("." + options.listItemLabelClass).slice(0,1).text()); 
