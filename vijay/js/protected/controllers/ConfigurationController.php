...
class ConfigurationController extends Controller
{
	...
	*/
    	...
    	if((isset($_GET['term'])) && (!empty($_GET['term'])))
       	{
			...
			//$criteria->condition = (("t.city_name LIKE :city_name") & (City.city_id=Country.city_id));
    ...
    public function actionProviderSearchCountry()
    {
    	// verify _GET param exist
    	if((isset($_GET['term'])) && (!empty($_GET['term'])))
    	{
    		...
    		// find all countries with criteria param
    		$criteria = new CDbCriteria;
    		$criteria->condition = "t.country_name LIKE :country_name";
    		$criteria->params = array(
    				":country_name" => '%'.CHtml::encode($_GET['term']).'%',
    		);
    		$criteria->limit = 5;
	...
	{
	/* this method displays the input form to create the project, which is at the end of this method, after the if....
	but it checks if a submit post action has been activated and starts the process of saving the information, first it creates an instance
	of the model, then it validates and saves, and also other actions like creating relations between user and projects.
	then finally when the saving is completed it displays a summary result of the newly created project based on the id 
	of this project*/
		...
		if ((Yii::app()->user->checkAccess('createProjects')) && (Yii::app()->user->checkAccess('permissionsConfiguration')) && (Yii::app()->user->IsAdministrator))
		{
			...
			if(isset($_POST['Projects']))
			{
				...
				if($model->save())
				{
					...
					// JMS the following line renders a view based on the id model..., but it uses the redirect instruction
			...
			// output create page
			// this is the view which also calls the input form
